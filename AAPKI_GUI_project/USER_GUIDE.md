...
## Overview
Aperture Photometry Toolkit is a GUI-based, step-by-step pipeline for FITS photometry. It
generates calibrated per-frame results, CMD tables, and QA reports while keeping the main
workflow (photometry -> CMD) separate from QA tools. For full algorithm details and a
complete parameter reference, see TECHNICAL_REFERENCE.md.
...
- Manual add/remove of master IDs on REF frame.
- Uses Gaia catalog overlay for visual selection.
- Outputs: `result/master_star_ids.csv`.

### Step 9 - Forced Photometry
- Performs photometry at fixed XY for each master ID.
- Uses aperture/annulus scales and sky clipping parameters.
...
### Step 12 - CMD Plot
- Opens CMD viewer using calibrated tables (embedded when the step opens).
- Uses `median_by_ID_filter_wide_cmd.csv`.

### Step 13 - Isochrone Model
- Isochrone fitting and interactive overlay on CMD.
