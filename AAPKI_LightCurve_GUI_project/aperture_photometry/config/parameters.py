...
def _read_toml(path: Path) -> Dict[str, Any]:
    ...
    def map_keys(mapping: Iterable[tuple[Iterable[str], str]]):
        for path_keys, out_key in mapping:
...
class Parameters:
    ...
    def _load_from_file(path: Path) -> types.SimpleNamespace:
        ...
        P = types.SimpleNamespace(
            ...
            night_parse_mode=raw.get("night_parse_mode", "regex"),
            night_parse_regex=raw.get("night_parse_regex", r".*_(\d{8})"),
            night_parse_split_delim=raw.get("night_parse_split_delim", "_"),
            night_parse_split_index=_geti(raw, "night_parse_split_index", -1),
            night_parse_last_digits=_geti(raw, "night_parse_last_digits", 8),
            night_parse_include_unmatched=_as_bool(raw.get("night_parse_include_unmatched", "false"), False),
            airmass_formula=raw.get("airmass_formula", "Kasten & Young (1989)"),
            airmass_update_header=_as_bool(raw.get("airmass_update_header", "false"), False),
            airmass_update_mode=raw.get("airmass_update_mode", "overwrite"),
            file_path_map={},
            target_name=raw.get("target_name", ""),
            target_ra_deg=_as_float_or_none(raw.get("target_ra_deg", "")),
            target_dec_deg=_as_float_or_none(raw.get("target_dec_deg", "")),
            ...
            extfit_order=_geti(raw, "extfit_order", 1),
            extfit_min_points=_geti(raw, "extfit_min_points", 5),
            extfit_clip_sigma=_getf(raw, "extfit_clip_sigma", 3.0),
            extfit_fit_iters=_geti(raw, "extfit_fit_iters", 5),
            extfit_use_color_terms=_as_bool(raw.get("extfit_use_color_terms", "false"), False),
            ...
            idmatch_init_r_fwhm=_getf(raw, "idmatch_init_r_fwhm", 5.0),
            idmatch_ratio_max=_getf(raw, "idmatch_ratio_max", 0.7),
            idmatch_min_pairs=_geti(raw, "idmatch_min_pairs", 15),
            idmatch_transform_mode=raw.get("idmatch_transform_mode", "similarity"),
            idmatch_mutual_nearest=_as_bool(raw.get("idmatch_mutual_nearest", "true"), True),

            # Reference build
            ref_select_sat_pct=_getf(raw, "ref_select_sat_pct", 20.0),
            ref_select_elong_pct=_getf(raw, "ref_select_elong_pct", 20.0),
            ref_per_date=_as_bool(raw.get("ref_per_date", "true"), True),
            ref_cat_max_sources=_geti(raw, "ref_cat_max_sources", 0),
            ref_cat_min_sources=_geti(raw, "ref_cat_min_sources", 50),
            ref_cat_max_elong=_getf(raw, "ref_cat_max_elong", 1.5),
            ref_cat_max_abs_round=_getf(raw, "ref_cat_max_abs_round", 0.4),
            ref_cat_sharp_min=_getf(raw, "ref_cat_sharp_min", 0.2),
            ref_cat_sharp_max=_getf(raw, "ref_cat_sharp_max", 1.0),
            ref_cat_min_peak_adu=_getf(raw, "ref_cat_min_peak_adu", 0.0),
            ref_wcs_match_radius_arcsec=_getf(raw, "ref_wcs_match_radius_arcsec", 2.0),
            ref_wcs_min_match_rate=_getf(raw, "ref_wcs_min_match_rate", 0.2),
            ref_wcs_min_match_n=_geti(raw, "ref_wcs_min_match_n", 50),
            ref_wcs_max_sep_med_arcsec=_getf(raw, "ref_wcs_max_sep_med_arcsec", 1.5),
            ref_wcs_max_sep_p90_arcsec=_getf(raw, "ref_wcs_max_sep_p90_arcsec", 2.5),
            ref_wcs_max_dup_rate=_getf(raw, "ref_wcs_max_dup_rate", 0.1),
            ...
            wcs_propagate_max_shift_px=_getf(raw, "wcs_propagate_max_shift_px", 50.0),
            wcs_propagate_min_match=_geti(raw, "wcs_propagate_min_match", 12),
            wcs_propagate_sigma_clip=_getf(raw, "wcs_propagate_sigma_clip", 3.0),
