# Technical Reference (AAPKI Light Curve GUI)

## Scope and Conventions
This document describes the scientific and processing logic for the AAPKI GUI
pipeline (Steps 1-12), plus the light curve GUI scaffolding that reuses those
outputs.
...
## Light Curve Modules (Scaffold)
- `aperture_photometry/analysis/light_curve/`: loaders and per-mode builders.
- `aperture_photometry/gui/light_curve/`: tab widgets for variable, eclipsing,
  and asteroid light curves.
- Workflow adds target/comparison selection and a light curve builder step.
...
### [background]
- `enable` (bool) 2D background enable (reserved; `in_detect` is used).
...
- `use_original_frames` (bool) reserved.
