...
## Architecture
- GUI entry point: `main.py` (loads `MainWindowLightCurve`).
- Core package: `aperture_photometry/`.
  ...
  - `gui/light_curve/`: tab widgets for light curve modes.
...
## Workflow
- Light curve workflow uses 12 steps in `main_window_workflow.py`.
- Target/Comparison selection feeds the forced photometry filter.
- State is persisted in `result/project_state.json` for the workflow path.
