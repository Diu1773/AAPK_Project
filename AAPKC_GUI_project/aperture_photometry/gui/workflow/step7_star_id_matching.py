...
def _coalesce_float(x, default: float) -> float:
    v = _safe_float(x, np.nan)
    return float(v) if np.isfinite(v) else float(default)


def _coalesce_int(x, default: int) -> int:
    try:
        if x is None:
            return int(default)
        s = str(x).strip()
...
def _signature_matches(saved_sig: dict, expected_sig: dict) -> bool:
    if not isinstance(saved_sig, dict) or not isinstance(expected_sig, dict):
        return False
    if bool(saved_sig.get("source_use_cropped", None)) != bool(expected_sig.get("source_use_cropped", None)):
        return False
    if _norm_path_key(saved_sig.get("source_path")) != _norm_path_key(expected_sig.get("source_path")):
        return False
    try:
        if int(saved_sig.get("source_mtime_ns")) != int(expected_sig.get("source_mtime_ns")):
            return False
    except Exception:
        return False
    return True
...
def _signature_matches_relaxed(saved_sig: dict, expected_sig: dict) -> bool:
    # Accept mtime drift when path/crop/size are stable.
    if _signature_matches(saved_sig, expected_sig):
        return True
    if not isinstance(saved_sig, dict) or not isinstance(expected_sig, dict):
        return False
    if bool(saved_sig.get("source_use_cropped", None)) != bool(expected_sig.get("source_use_cropped", None)):
    ...
    try:
        ...
        expected_size = int(expected_sig.get("source_size"))
    except Exception:
        return False
    if saved_size <= 0 or expected_size <= 0:
        return False
    return saved_size == expected_size
...
class IdMatchWorker(QThread):
    ...
    def __init__(
        ...
        self.match_r_fwhm = max(0.1, _coalesce_float(match_r_fwhm, 0.8))
        self.init_r_fwhm = max(0.5, _coalesce_float(init_r_fwhm, 5.0))
        self.ratio_max = min(1.0, max(0.0, _coalesce_float(ratio_max, 0.7)))
        self.min_pairs = max(1, _coalesce_int(min_pairs, 15))
        ...
        self.tight_radius_arcsec = max(0.2, _coalesce_float(tight_radius_arcsec, 1.0))
        self.loose_radius_arcsec = max(self.tight_radius_arcsec, _coalesce_float(loose_radius_arcsec, 3.0))
        self.min_correction_pairs = max(2, _coalesce_int(min_correction_pairs, 3))
        self.min_affine_pairs = max(3, _coalesce_int(min_affine_pairs, 6))
        self.adaptive_retry_threshold = max(0.0, _coalesce_float(adaptive_retry_threshold, 0.5))
        ...
        self._detect_csv_missing_warned: set[str] = set()
        self._detect_csv_invalid_warned: set[str] = set()
        self._detect_meta_fallback_warned: set[str] = set()
        self._ref_filter_logged: set[str] = set()
        self.use_gaia_refs_only = bool(getattr(self.params.P, "idmatch_use_gaia_refs_only", False))
        self.gaia_g_limit = _safe_float(getattr(self.params.P, "idmatch_gaia_g_limit", np.nan), np.nan)
    ...
    def _current_file_signature(self, fname: str) -> Optional[dict]:
        ...
        if path is None or not path.exists():
            return None
        return _build_file_signature(path, use_cropped=bool(crop_is_active(self.result_dir)))
    ...
    def _detect_meta_compatible(self, fname: str, payload: dict, meta_path: Path) -> bool:
        ...
        if sig_now is None:
            return False
        if _signature_matches(payload, sig_now):
            payload["__compat_relaxed_mtime"] = False
            payload["__compat_relaxed_size"] = False
            return True
        if not _signature_matches_relaxed(payload, sig_now):
            return False
    ...
    def _fallback_fwhm_px(self, pix_scale_arcsec: float = np.nan) -> float:
        fwhm_px = _safe_float(getattr(self.params.P, "fwhm_pix_guess", None), np.nan)
        if np.isfinite(fwhm_px) and fwhm_px > 0:
            return float(fwhm_px)
        fwhm_arc = _safe_float(getattr(self.params.P, "fwhm_guess_arcsec", None), np.nan)
        pix_cfg = _safe_float(getattr(self.params.P, "pixel_scale_arcsec", None), np.nan)
        if (not np.isfinite(pix_cfg) or pix_cfg <= 0) and np.isfinite(pix_scale_arcsec) and pix_scale_arcsec > 0:
            pix_cfg = float(pix_scale_arcsec)
        if np.isfinite(fwhm_arc) and fwhm_arc > 0 and np.isfinite(pix_cfg) and pix_cfg > 0:
            return float(fwhm_arc / pix_cfg)
        return 6.0
    ...
    def _filter_ref_catalog_for_matching(self, df: pd.DataFrame, label: str) -> pd.DataFrame:
        if df is None or df.empty:
            return df
        out = df.copy()
        if not self.use_gaia_refs_only:
            return out

        sid = parse_int64_series(out["source_id"]) if "source_id" in out.columns else pd.Series(dtype="Int64")
        gaia_mask = sid.notna() & (sid > 0)
        out = out.loc[gaia_mask].copy()

        gcol = None
        for cand in ("gaia_G", "phot_g_mean_mag"):
            if cand in out.columns:
                gcol = cand
                break
        if gcol is not None and np.isfinite(self.gaia_g_limit) and self.gaia_g_limit > 0:
            gmag = pd.to_numeric(out[gcol], errors="coerce")
            out = out.loc[gmag.isna() | (gmag <= float(self.gaia_g_limit))].copy()

        frac = float(len(out) / max(len(df), 1))
        min_keep = 30
        if len(out) < min_keep:
            if label not in self._ref_filter_logged:
                self._ref_filter_logged.add(label)
                self._log(
                    f"[IDMATCH][REF] {label}: Gaia-only refs too few ({len(out)}/{len(df)}). "
                    "fallback to full ref catalog."
                )
            return df

        if label not in self._ref_filter_logged:
            self._ref_filter_logged.add(label)
            limit_txt = f"{self.gaia_g_limit:.2f}" if np.isfinite(self.gaia_g_limit) and self.gaia_g_limit > 0 else "off"
            self._log(
                f"[IDMATCH][REF] {label}: Gaia-only refs {len(out)}/{len(df)} kept "
                f"(source_id>0, gaia_g_limit={limit_txt})"
            )
            if frac < 0.25:
                self._log(
                    f"[IDMATCH][REF][WARN] Gaia-only reference fraction is low ({frac:.1%}). "
                    "For dense clusters, disable Gaia-only mode to use local (non-Gaia) refs."
                )
        return out
    ...
    def _get_ref_for_date(self, date_key: str, master_df: pd.DataFrame) -> Tuple[np.ndarray, SkyCoord]:
        ...
        try:
            ...
            df = self._filter_ref_catalog_for_matching(df, f"date={cache_key}")
            if df.empty:
    ...
    def _run_impl(self):
        ...
        gl_txt = f"{self.gaia_g_limit:.2f}" if np.isfinite(self.gaia_g_limit) and self.gaia_g_limit > 0 else "off"
        self._log(
            f"[IDMATCH] ref policy: gaia_only={self.use_gaia_refs_only} gaia_g_limit={gl_txt}"
        )
        ...
        master_df = self._filter_ref_catalog_for_matching(master_df, "global")
        global_ref_ids = master_df["source_id"].astype("int64").to_numpy()
        global_ref_sky = SkyCoord(master_df["ra_deg"].to_numpy(float) * u.deg,
                                  master_df["dec_deg"].to_numpy(float) * u.deg,
                                  frame="icrs")
        if self.use_gaia_refs_only:
            self._log(
                f"[IDMATCH][REF] Gaia-only cap: max possible matched IDs per frame <= {len(global_ref_ids)}"
            )
        ...
        for k, fname in enumerate(self.file_list, 1):
            ...
            if not np.isfinite(fwhm_px) or fwhm_px <= 0:
                fwhm_px = self._fallback_fwhm_px(pix_scale)

            ref_ids, ref_sky = self._get_ref_for_date(date_key, master_df)

            if n_det == 0:
                if np.isfinite(match_radius_arcsec) and match_radius_arcsec > 0:
                    match_r_zero = float(match_radius_arcsec)
                elif self.two_pass_enable:
                    match_r_zero = float(self.loose_radius_arcsec)
                elif np.isfinite(pix_scale) and pix_scale > 0:
                    match_r_zero = float(self.match_r_fwhm * fwhm_px * pix_scale)
                else:
                    match_r_zero = 2.0
                if n_det > 0:
                    df_out = pd.DataFrame({
                ...
                stat = {
                    ...
                    "match_rate": 0.0,
                    "match_radius_arcsec": float(match_r_zero),
            ...
            if det_sky is None:
                ...
                if np.isfinite(match_radius_arcsec) and match_radius_arcsec > 0:
                    ...
                    if self.two_pass_enable:
                        match_r = float(self.loose_radius_arcsec)
                    elif np.isfinite(pix_scale) and pix_scale > 0:
                        match_r = float(self.match_r_fwhm * fwhm_px * pix_scale)
                    else:
                        fwhm_arc = _safe_float(getattr(self.params.P, "fwhm_guess_arcsec", None), np.nan)
                        if np.isfinite(fwhm_arc) and fwhm_arc > 0:
                            match_r = float(self.match_r_fwhm * fwhm_arc)
                        else:
                            match_r = 2.0
        ...
        try:
            ...
            sid2id["source_id"] = parse_int64_series(sid2id["source_id"])
            sid2id["ID"] = pd.to_numeric(sid2id["ID"], errors="coerce")
            ...
            for fname in self.file_list:
                ...
                if "source_id" not in df_fm.columns or "x" not in df_fm.columns:
                    continue
                df_fm = df_fm.copy()
                df_fm["source_id"] = parse_int64_series(df_fm["source_id"])
...
class StarIdMatchingWindow(StepWindowBase):
    ...
    def open_parameters_dialog(self):
        ...
        arcsec_spin.setValue(_coalesce_float(getattr(self.params.P, "idmatch_tol_arcsec", None), 0.0))
        ...
        match_r_spin.setValue(max(0.1, _coalesce_float(getattr(self.params.P, "idmatch_match_r_fwhm", None), 0.8)))
        match_r_spin.setToolTip("FWHM multiplier for auto match radius.")
        form.addRow("Fallback radius (FWHM ×):", match_r_spin)

        gaia_only_chk = QCheckBox("Use Gaia-matched refs only")
        gaia_only_chk.setChecked(bool(getattr(self.params.P, "idmatch_use_gaia_refs_only", False)))
        form.addRow("Reference set:", gaia_only_chk)

        gaia_g_spin = QDoubleSpinBox()
    ...
    def run_idmatch(self):
        ...
        solved_path = next((p for p in solved_candidates if p.exists()), None)
        if solved_path is not None:
            try:
                df_sol = pd.read_csv(solved_path)
            except Exception as e:
                self.log(f"[WCS] Could not read solved summary ({solved_path.name}): {e}")
                df_sol = pd.DataFrame()
            if not df_sol.empty:
                key_col = "file" if "file" in df_sol.columns else ("fname" if "fname" in df_sol.columns else None)
                ok_col = "wcs_ok" if "wcs_ok" in df_sol.columns else ("ok" if "ok" in df_sol.columns else None)
                if key_col is not None and ok_col is not None:
                    ok_vals = df_sol[ok_col].astype(str).str.strip().str.lower().isin({"true", "1", "yes", "y", "on"})
                    solved_set = set(df_sol.loc[ok_vals, key_col].astype(str).tolist())
                    before_n = len(files)
                    files = [f for f in files if f in solved_set]
                    self.log(f"[WCS] solved-only filter: {len(files)}/{before_n} kept ({solved_path.name})")
                else:
                    self.log(f"[WCS] solved summary missing key columns: {solved_path.name}")
        if not files:
            QMessageBox.warning(self, "Warning", "No WCS-solved frames available for ID matching.")
            return
