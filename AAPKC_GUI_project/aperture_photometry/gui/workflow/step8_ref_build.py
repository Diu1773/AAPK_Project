...
class RefBuildWorker(QThread):
    ...
    def _wcs_row_compatible(self, fname: str, row: dict) -> bool:
        ...
        if src_path_txt not in ("", "nan", "none", "null"):
            if _signature_matches(row, sig_now):
                row["__compat_relaxed_mtime"] = False
                row["__compat_relaxed_size"] = False
                return True
            if not _signature_matches_relaxed(row, sig_now):
                return False
        ...
        fits_path = _norm_path_key(row.get("fits_path", ""))
        if not fits_path:
            return False
    ...
    def _apply_hybrid_source_ids(self, df: pd.DataFrame, gaia_mag_limit: float = 18.0) -> tuple[pd.DataFrame, dict[int, int], dict[int, int]]:
        ...
        if len(out) > 0:
            frac = float(n_gaia / max(len(out), 1))
            if frac < 0.10:
                self._log(
    ...
    def _load_wcs_meta(self, fname: str) -> dict:
        ...
        try:
            ...
            return {}
        try:
            payload["__record_mtime_ns"] = int(meta_path.stat().st_mtime_ns)
        except Exception:
            payload["__record_mtime_ns"] = 0
        if not self._wcs_row_compatible(fname, payload):
            return {}
        return payload
    ...
    def _load_wcs_summary_map(self) -> Dict[str, dict]:
        ...
        for path in existing:
            ...
            try:
                ...
                continue
            try:
                rec_mtime_ns = int(path.stat().st_mtime_ns)
            except Exception:
                rec_mtime_ns = 0
            key_col = None
    ...
    def _frame_metrics(self, fname: str) -> Optional[dict]:
        ...
        if not filt:
            ...
            if filt == "unknown":
                self._log(f"[REF][QC] {fname}: filter unresolved, using 'unknown'")
    ...
    def _select_reference(self, metrics: pd.DataFrame, ref_filter: str) -> str:
        ...
        if rate_cols:
            cand["match_rate_eff"] = pd.concat(
                [pd.to_numeric(cand[c], errors="coerce") for c in rate_cols],
                axis=1,
        ...
        cand_wcs_base = cand.copy()
        if "wcs_ok" in cand_wcs_base.columns:
            cand_wcs_base = cand_wcs_base[cand_wcs_base["wcs_ok"] == True]
        n_wcs_valid = len(cand_wcs_base)

        cand_wcs = cand_wcs_base.copy()
        ...
        self._log(
            "[REF][QC] wcs thresholds: match_r={r:.2f}\" min_eff_rate={mr:.2f} "
    ...
    def _run_impl(self):
        ...
        for row in metrics.to_dict(orient="records"):
            ...
            wcs_sum = self._load_wcs_summary_row(fname)

            row["wcs_resid_med"] = _safe_float(wcs_meta.get("resid_med"), np.nan)
            if not np.isfinite(row["wcs_resid_med"]):
                row["wcs_resid_med"] = _safe_float(wcs_sum.get("resid_med"), np.nan)
            if not np.isfinite(row["wcs_resid_med"]):
                resid_px = _safe_float(wcs_sum.get("resid_med_px"), np.nan)
                pix_arc = _safe_float(wcs_sum.get("pix_scale_fit_arcsec"), np.nan)
                if not np.isfinite(pix_arc) or pix_arc <= 0:
                    pix_arc = _safe_float(wcs_sum.get("pix_fit"), np.nan)
                if not np.isfinite(pix_arc) or pix_arc <= 0:
                    pix_arc = _safe_float(getattr(self.params.P, "pixel_scale_arcsec", np.nan), np.nan)
                if np.isfinite(resid_px) and np.isfinite(pix_arc) and pix_arc > 0:
                    row["wcs_resid_med"] = float(resid_px * pix_arc)

            row["wcs_resid_max"] = _safe_float(wcs_meta.get("resid_max"), np.nan)
            if not np.isfinite(row["wcs_resid_max"]):
                row["wcs_resid_max"] = _safe_float(wcs_sum.get("resid_max"), np.nan)

            row["wcs_match_n"] = int(wcs_meta.get("match_n", 0) or 0)
            if row["wcs_match_n"] <= 0:
                row["wcs_match_n"] = int(wcs_sum.get("match_n", 0) or 0)
            ...
            if not np.isfinite(_safe_float(row.get("match_rate"), np.nan)):
                row["match_rate"] = _safe_float(wcs_sum.get("match_rate"), np.nan)
            if not np.isfinite(_safe_float(row.get("match_rate_cat"), np.nan)):
                row["match_rate_cat"] = _safe_float(wcs_sum.get("match_rate_cat"), np.nan)
            if not np.isfinite(_safe_float(row.get("match_rate_eff"), np.nan)):
                row["match_rate_eff"] = _safe_float(wcs_sum.get("match_rate_eff"), np.nan)
            ...
            row["match_rate_eff"] = float(max(mr_candidates)) if mr_candidates else np.nan

            if int(row.get("wcs_match_n", 0) or 0) <= 0:
                row["wcs_match_n"] = int(row.get("n_match", 0) or 0)
            if not np.isfinite(_safe_float(row.get("wcs_resid_med"), np.nan)):
                sep_med = _safe_float(row.get("sep_med_arcsec"), np.nan)
                if np.isfinite(sep_med):
                    row["wcs_resid_med"] = float(sep_med)
        ...
        if "wcs_ok" in metrics.columns:
            try:
                wcs_ok_count = int(metrics["wcs_ok"].fillna(False).astype(bool).sum())
            except Exception:
                wcs_ok_count = None
        if wcs_ok_count is not None and wcs_ok_count <= 0:
            summary_hint = step5_dir(self.result_dir) / "wcs_solve_summary.csv"
            if not summary_hint.exists():
                legacy_hint = legacy_step7_wcs_dir(self.result_dir) / "wcs_solve_summary.csv"
        ...
        # AAPKC compat: master_catalog.tsv (alias of ref_catalog.tsv)
        master_df.to_csv(
            out_dir / "master_catalog.tsv",
            sep="\t",
            index=False,
            na_rep="NaN",
            encoding="utf-8-sig",
        )
        # AAPKC compat: master_star_ids.csv (source_id list for step8)
        master_df[["source_id"]].to_csv(
            out_dir / "master_star_ids.csv", index=False, encoding="utf-8-sig"
        )
        # Backup for step8 MasterIdEditor
        master_df[["source_id"]].to_csv(
            out_dir / "master_star_ids.orig.csv", index=False, encoding="utf-8-sig"
        )
...
class RefBuildWindow(StepWindowBase):
    ...
    def check_detection_status(self):
        ...
        if crop_is_active(self.params.P.result_dir):
            cdir = step2_cropped_dir(self.params.P.result_dir)
            if cdir.exists():
                expected_files = {p.name for p in cdir.glob("*.fit*")}
        if not expected_files and self.file_manager is not None:
            try:
        ...
        metas = [
            p for p in cache_dir.glob("detect_*.json")
            if not (p.name.startswith("detect_prev_") or p.name.startswith("detect_tmp_"))
        ]
        if not metas:
            step4_out = step4_dir(self.params.P.result_dir)
            metas = [
                p for p in step4_out.glob("detect_*.json")
                if not (p.name.startswith("detect_prev_") or p.name.startswith("detect_tmp_"))
            ]
        if not metas:
        ...
        matched = 0
        legacy_matched = 0
        use_cropped_now = bool(crop_is_active(self.params.P.result_dir))
        rect_mtime_ns = 0
        if use_cropped_now:
            rect_path = crop_rect_path(self.params.P.result_dir)
            if rect_path.exists():
                try:
                    rect_mtime_ns = int(rect_path.stat().st_mtime_ns)
                except Exception:
                    rect_mtime_ns = 0
        for p in metas:
            name = p.name
            if not (name.startswith("detect_") and name.endswith(".json")):
                continue
            fname = name[len("detect_"):-len(".json")]
            if (not expected_files) or (fname in expected_files):
                try:
                ...
                if schema < 2:
                    try:
                        ...
                        mtime_ns = 0
                    if rect_mtime_ns > 0 and mtime_ns < rect_mtime_ns:
                        continue
                    legacy_matched += 1
                else:
                    if bool(payload.get("source_use_cropped", None)) != use_cropped_now:
                        continue
                    matched += 1
        if matched <= 0:
            if legacy_matched > 0:
                self.status_label.setText(
                    f"Detection cache found: {legacy_matched} frames (legacy; re-run Step4 recommended)"
                )
                self.status_label.setStyleSheet("color: #ef6c00;")
                return
            self.status_label.setText("Detection cache exists but does not match current frame set.")
            self.status_label.setStyleSheet("color: #ef6c00;")
            return
        if legacy_matched > 0:
            self.status_label.setText(
                f"Detection cache found: {matched} frames (+{legacy_matched} legacy)"
            )
            self.status_label.setStyleSheet("color: #ef6c00;")
        else:
            self.status_label.setText(f"Detection cache found: {matched} frames")
            self.status_label.setStyleSheet("color: green;")

    def _load_existing_outputs(self):
        out_dir = step6_dir(self.params.P.result_dir)
        stats_path = out_dir / "ref_frame_stats.csv"
        wcs_summary_candidates = [
        ...
        wcs_summary_path = next((p for p in wcs_summary_candidates if p.exists()), None)
        if stats_path.exists() and wcs_summary_path is not None:
            try:
                if stats_path.stat().st_mtime_ns < wcs_summary_path.stat().st_mtime_ns:
                    self.results = {}
        ...
        meta_candidates = [
            out_dir / "ref_build_meta.json",
            self.params.P.result_dir / "ref_build_meta.json",
        ]
        summary = {}
        for mpath in meta_candidates:
            ...
            summary = {
                ...
                "n_sources": int(meta.get("n_ref_used", meta.get("n_ref_total", 0)) or 0),
                "ref_frames_by_date": meta.get("ref_frames_by_date", {}) or {},
                "ref_per_date": bool(meta.get("ref_per_date", False)),
            }
            break

        if (not summary) and stats_path.exists():
            try:
                df = pd.read_csv(stats_path)
                if not df.empty:
                    sel = df[df.get("selected", False) == True] if "selected" in df.columns else df.iloc[[0]]
                    if sel.empty:
                        sel = df.iloc[[0]]
                    first = sel.iloc[0]
                    n_src = pd.to_numeric(first.get("n_sources", 0), errors="coerce")
                    summary = {
                        "ref_frame": str(first.get("file", "")),
                        "ref_filter": str(first.get("filter", "")),
                        "n_sources": int(n_src) if np.isfinite(n_src) else 0,
                        "ref_frames_by_date": {},
                        "ref_per_date": False,
                    }
            except Exception:
        ...
        self._update_plot_tab(summary if summary else None)
    ...
    def _update_plot_tab(self, summary: Optional[dict] = None) -> None:
        ...
        if "match_rate_cat" in df.columns:
            df["match_rate_cat"] = pd.to_numeric(df["match_rate_cat"], errors="coerce")
        else:
            df["match_rate_cat"] = np.nan
        if "match_rate_eff" in df.columns:
            df["match_rate_eff"] = pd.to_numeric(df["match_rate_eff"], errors="coerce")
        else:
            df["match_rate_eff"] = np.nan
        df["match_rate_eff"] = pd.concat(
            [df["match_rate"], df["match_rate_cat"], df["match_rate_eff"]],
            axis=1,
        ).max(axis=1, skipna=True)
        ...
        for _, fname in selected_frames.items():
            ...
            r = row.iloc[0]
            ax1.scatter(
                r["sep_med_arcsec"], r["match_rate_eff"],
