...
_FILTER_ALIAS_MAP = {
    # Canonical SDSS names
    "u": "u", "g": "g", "r": "r", "i": "i", "z": "z", "y": "y",
    # Prime-style aliases
    "up": "u", "gp": "g", "rp": "r", "ip": "i", "zp": "z", "yp": "y",
    "uprime": "u", "gprime": "g", "rprime": "r", "iprime": "i", "zprime": "z", "yprime": "y",
    # SDSS/Sloan aliases commonly found in FITS headers
    "sdssu": "u", "sdssg": "g", "sdssr": "r", "sdssi": "i", "sdssz": "z",
    "sloanu": "u", "sloang": "g", "sloanr": "r", "sloani": "i", "sloanz": "z",
    # Band label aliases
    "uband": "u", "gband": "g", "rband": "r", "iband": "i", "zband": "z", "yband": "y",
}


def normalize_filter_name(value: object) -> str:
    """
    Normalize FITS/header filter strings into a stable key.

    Examples:
    - "g", "G", "g'", "g-prime", "SDSS-G" -> "g"
    - Unknown/custom filters are returned as lowercase stripped text.
    """
    if value is None:
        return ""
    ...
    # Unify common quote variants used to denote prime filters.
    raw = (
        raw.replace("′", "'")
        .replace("’", "'")
        .replace("`", "'")
    ...
    if compact in _FILTER_ALIAS_MAP:
        return _FILTER_ALIAS_MAP[compact]

    # Generic fallback for compact forms like "sdssgprime", "sloanr", etc.
    m = re.fullmatch(r"(?:sdss|sloan)?([ugrizy])(?:p|prime)?", compact)
    if m:
        return m.group(1)
